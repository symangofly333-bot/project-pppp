...
> **상태: 초안.** 배우기용 프롬프트는 실제 API로 시험함(`prompt-eval/`, 개별 항목은 4절 참고).
> 아래 `[DRAFT EXAMPLE]`로 표시된 예시 문구는 전부 확정된 것이 아니라 검토용으로 작성한 것이며,
> 블록 단위로 교체 가능하다.
>
> **`0-1`절 — 한 턴 문제는 해소됨.** 대화 기록 배관이 완료돼 이제 여러 턴이 실제로 이어진다.
> 다만 최근 10개(사용자+AI 합쳐서)로 자른다는 제약은 남아 있다.
...
## 0-1. ✅ 한 턴 문제 해소 — 대화 기록 배관 완료

과거엔 `chatbot.js`가 문장 하나만 보내고 서버도 문자열 하나만 받아서, 매 질문이
서로 완전히 독립적이었다. 지금은 고쳐졌다:

- `chatgpt-tutorial-extension/chatbot.js` — `history` 배열(최근 10개, 사용자+AI 합쳐서)을 들고
  있다가 `{ type: "ask-ai", messages: history }`로 전체를 보낸다
- `server/index.js` — `payload.messages` 배열을 받아 형식을 검사한다(`role`/`content`,
  첫 메시지는 user, role 교대)
- `server/ask-claude.js` — 새 모듈. `PROMPTS.md` 1절(배우기용) 프롬프트로 자연어 호출한다.
  JSON 파이프라인(`generate-validated.js`)은 더 이상 안 쓴다

3턴 대화로 이름을 기억하는지 라이브 확인 완료. 이제 아래 규칙들이 실제로 여러 턴에 걸쳐 시험 가능하다:

| 프롬프트 규칙 | 이제 되는 것 |
|---|---|
| 배우기용 `PREDICTION` — "1. 예측할래요 / 2. 그냥 보여줘" | 사용자가 "1"이라 답하면 앞 메시지가 같이 가서 뭘 가리키는지 안다 |
| 메타 교육 — "최근에 했으면 생략" | "최근"을 최근 10개 안에서 판단할 근거가 있다 |
| "필요한 정보 하나만 되물어라" | 사용자가 답하면 원래 질문이 기록에 남아 있다 |

**남은 제약:** 최근 10개를 넘어가는 대화는 여전히 잘린다. 그리고 지금 배관은
**배우기용 AI 한정**이다 — 코딩 팝업 AI는 진입점 자체가 없어 이 배관을 안 탄다(9절 참고).
코딩 `BEFORE YOU WRITE OR CHANGE CODE`의 1/2/3 선택은 아직 실제로 못 이어간다.
...
- **12번**은 이제 앱(`chatbot.js`)으로는 성립한다 — 대화 기록 배관이 됐으니 실제 팝업에서
  대화하며 눈으로 확인 가능하다. 다만 `prompt-eval/`(자동 시험 스크립트)는 아직 케이스마다
  메시지 하나만 보내는 구조라 — 앞 턴을 미리 채워 보내는 기능이 없다 — 자동화된 재현은
  `cases.js`에 다중 턴 지원을 추가해야 한다. **미착수.**
